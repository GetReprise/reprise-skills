...
Never send the `chrome-extension://` URL as a clickable `<a href>` — Chrome blocks the click. Full per-tier mechanics, race-condition details, and stranded-tab recovery in `tour_docs(slug='tour')`.

Don't proceed past `tour_capture_toolbar_mount` until status confirms pairing — commands to an unpaired session silently no-op. If the user doesn't have the extension installed, send them to the Chrome Web Store URL in `tour_docs(slug='tour')`.
...
1. `tour_screen_node_list(snapshot_id=<from tour_screen_list>)` — returns up to 200 anchorable candidates (interactive elements, headings, aria-labeled nodes) with `tag` / `text` / `role` / `aria_label`. Pick the node whose `text` or `aria_label` matches the feature the guide describes.
2. `tour_guide_create(draft_id=..., snapshot_id=..., guide_type='tethered', target_node_id=<from step 1>, text='<h2>Title</h2><p>Body</p>', buttons='[{"label":"Next","action":"flow_next_screen"}]')`. The `position` field auto-defaults sensibly (the server walks ancestors for header/nav and places the popup below if found, above otherwise) — only override when the default clips off-viewport.
